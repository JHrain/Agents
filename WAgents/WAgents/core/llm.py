"""WAgents统一LLM接口 - 基于OpenAI原生API"""

import os
from typing import Iterator, Literal, Optional
from openai import OpenAI

from .Exceptions import WAgentsException
...
class WAgents_LLM:
    """
    一个自定义的LLM客户端，它用于调用任何兼容OpenAI接口的服务，并默认使用流式响应。
    """
    ...
    def __init__(
        ...
        if not all([self.api_key, self.base_url]):
            raise WAgentsException("API密钥和服务地址必须被提供或在.env文件中定义。")
